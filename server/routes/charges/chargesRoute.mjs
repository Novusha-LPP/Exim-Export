...
router.get('/get-empty-yard-codes', async (req, res) => {
    try {
        const items = await EmptyYardDirectory.find().sort({ name: 1 });
        const mapped = items.map(i => {
            const obj = i.toObject();
            return {
                ...obj,
...
router.post('/reject-purchase-entry', async (req, res) => {
    try {
        ...
        // Also update the status of the Purchase Book entry in the database
        await PurchaseBookEntryModel.updateOne({ entryNo: requestNo }, { status: 'Rejected' });
