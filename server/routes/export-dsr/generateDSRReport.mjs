...
router.get("/api/export-dsr/generate-dsr-report", async (req, res) => {
  try {
    ...
    worksheet.columns = [
      {
        header: "Container placement date",
        key: "container_placement_date",
        width: 22,
      },
      {
        header: "Origin docs received",
        key: "origin_docs_received",
        width: 20,
      },
      { header: "Handover date", key: "handover_date", width: 18 },
      {
        header: "Gate in at thar/khodiyar date & time",
        key: "gate_in_thar",
        width: 30,
      },
      {
        header: "Rail out date from icd (planned)",
        key: "rail_out_planned",
        width: 30,
      },
    ...
    headerRow.height = 60; // Increased height significantly for better readability
    ...
    // Set widths for all columns - increasing base widths
    worksheet.getColumn("container_placement_date").width = 15;
    worksheet.getColumn("origin_docs_received").width = 15;
    worksheet.getColumn("handover_date").width = 15;
    worksheet.getColumn("gate_in_thar").width = 20;
    worksheet.getColumn("rail_out_planned").width = 20;
    worksheet.getColumn("rail_out_actual").width = 15;
    worksheet.getColumn("cntr_port_gate_in").width = 20;
    worksheet.getColumn("remarks").width = 15;
    worksheet.getColumn("consignment_type").width = 12;
    worksheet.getColumn("milestone_remarks").width = 20;
    ...
    worksheet.getColumn("sb_no").width = 20;
    worksheet.getColumn("sb_date").width = 15;
