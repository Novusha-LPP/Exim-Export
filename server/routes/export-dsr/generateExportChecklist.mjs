...
export const generateExportChecklist = async (jobNumber) => {
  try {
    ...
    const drawHeader = (pageNum) => {
      ...
      doc.text(exportJob.companyName || "SURAJ FORWARDERS & SHIPPING AGENCIES", 0, 15, { align: "center", width: doc.page.width });
    ...
    drawFieldBox(10, yPos, 170, 18, "SB No. / Date", `${exportJob.shippingbillnumber} dt ${exportJob.shippingbilldate}`, 6, 5);
    ...
    drawFieldBox(185, yPos, 120, 18, "Nature of Cargo", exportJob.natureofcargo, 6, 5);
    drawFieldBox(310, yPos, 275, 18, "Port Of Loading", exportJob.portofloading, 6, 5);
    yPos += 20;
    ...
    drawFieldBox(200, yPos, 50, 18, "No Of Cntnrs", exportJob.noofcontainers, 6, 5);
    yPos += 20;
    ...
    drawFieldBox(340, yPos, 60, 18, "Insurance", exportJob.insuranceAmount, 6, 5);
    yPos += 20;
    ...
    const itemHeaders = ["SI No", "Qty", "Unit", "RITC", "Exim Scheme Code description", "NFEI Catg", "Reward Item", "Description", "Unit Price Unit", "FOB ValFC", "FOB ValINR", "Total ValueFC", "IGST Pymt Status", "PMVUnit", "IGST Taxable Value", "Total PMVINR", "IGST Amount"];
    const colX = [10, 35, 60, 90, 130, 200, 240, 270, 370, 430, 470, 510, 540, 570, 600];
    const colWidths = [25, 25, 30, 40, 70, 40, 30, 100, 60, 40, 40, 30, 30, 30, 30, 40, 40];
    ...
    for (let i = 0; i < itemHeaders.length; i++) {
      if (colX[i] && colWidths[i]) {
        ...
        doc.text(itemHeaders[i], colX[i] + 1, yPos + 2, { width: colWidths[i] - 2 });
      }
    }
    yPos += 40;
    ...
    const dbkHeaders = ["Inv No", "Item No", "DBK SI No", "Custom Rate", "DBK Rate", "DBK Qty Unit", "DBK Amount", "Custom SPE", "DBK SPE"];
    ...
    drawFieldBox(10, yPos, 90, 16, "Factory Stuffed", exportJob.goodsstuffedat === "Factory" ? "Yes" : "No", 6, 5);
    drawFieldBox(105, yPos, 90, 16, "Seal Type", exportJob.stuffingsealtype, 6, 5);
    drawFieldBox(200, yPos, 180, 16, "Vessel Name", exportJob.shippinglineairline, 6, 5);
    drawFieldBox(385, yPos, 200, 16, "Voyage Number", exportJob.voyageno, 6, 5);
    yPos += 20;

    yPos = drawSectionHeader(yPos, "CONTAINER DETAILS");
    const contHeaders = ["Container No", "Size", "Type", "Seal No", "Seal Type", "Seal Date", "Seal Device ID"];
    ...
    const suppHeaders = [
      "Inv/Item/SrNo.", "Doc Issue Date", "Doc Expiry Date", "Doc Type Code",
      "Image Ref.No.(IRN)", "Doc Ref.No.", "Doc Uploaded On", "Doc Name",
      "ICEGATE ID", "File Type", "Place of Issue", "Issuing Party Code", "Beneficiary Party Code",
      "Issuing Party Name", "Issuing Party Add1", "Issuing Party Add2", "Issuing Party City", "Issuing Party Pin Code",
      "Beneficiary Party Name", "Beneficiary Party Add1", "Beneficiary Party Add2", "Beneficiary Party City", "Beneficiary Party Pin Code"
    ];
    ...
    suppHeaders.forEach((header, i) => {
      ...
      const row = i % 4;
      if (suppColX[col]) {
        doc.rect(suppColX[col], yPos + (row * 12), suppColWidths[col], 12).stroke();
        doc.text(header, suppColX[col] + 1, yPos + (row * 12) + 2, { width: suppColWidths[col] - 2 });
      }
    });
    yPos += 48;
    ...
    supportingDocs.forEach(sDoc => {
      ...
      docRow.slice(0, 9).forEach((data, i) => {
        ...
        doc.text(data || "", suppColX[i] + 1, yPos + 2, { width: suppColWidths[i] - 2 });
      });
      yPos += 10;
    });
