...
import { ShippingLineService } from "./MasterDirectoryService";
...
const ShippingLineMaster = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await ShippingLineService.getAll();
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await ShippingLineService.update(selectedItem._id, formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this entry?")) {
      try {
        await ShippingLineService.delete(id);
  ...
  return (
    <Box>
      <Paper sx={{ p: 2, mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <TextField
          size="small" placeholder="Search Shipping Lines..."
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
        <DialogContent sx={{ p: 0 }}>
          <MasterDirectoryForm 
            title={selectedItem ? "Edit Shipping Line" : "Add Shipping Line"}
            data={selectedItem} 
            onSave={handleSave} 
