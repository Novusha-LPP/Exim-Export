...
import { EmptyYardCodeService } from "./MasterDirectoryService";
import MasterDirectoryForm from "./MasterDirectoryForm";

const EmptyYardCodeMaster = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await EmptyYardCodeService.getAll({
        page,
        limit: 10,
        search: searchTerm
      });
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await EmptyYardCodeService.update(selectedItem._id, formData);
        showSnackbar("Updated successfully");
      } else {
        await EmptyYardCodeService.create(formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this Empty Yard entry?")) {
      try {
        await EmptyYardCodeService.delete(id);
  ...
  return (
    <Box>
      <Paper sx={{ p: 2, mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <TextField
          size="small" placeholder="Search Empty Yards..."
          value={searchTerm} 
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
        <DialogContent sx={{ p: 0 }}>
          <MasterDirectoryForm 
            title={selectedItem ? "Edit Empty Yard" : "Add Empty Yard"}
            data={selectedItem} 
            onSave={handleSave} 
...
export default EmptyYardCodeMaster;
