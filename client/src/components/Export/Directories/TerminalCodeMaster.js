...
import { TerminalCodeService } from "./MasterDirectoryService";
import MasterDirectoryForm from "./MasterDirectoryForm";

const TerminalCodeMaster = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await TerminalCodeService.getAll();
  ...
  const handleSave = async (formData) => {
    try {
      if (selectedItem) {
        await TerminalCodeService.update(selectedItem._id, formData);
        showSnackbar("Updated successfully");
      } else {
        await TerminalCodeService.create(formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this entry?")) {
      try {
        await TerminalCodeService.delete(id);
  ...
  return (
    <Box>
      <Paper sx={{ p: 2, mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <TextField
          size="small" placeholder="Search Terminal Codes..."
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
        <DialogContent sx={{ p: 0 }}>
          <MasterDirectoryForm 
            title={selectedItem ? "Edit Terminal Code" : "Add Terminal Code"}
            data={selectedItem} 
            onSave={handleSave} 
...
export default TerminalCodeMaster;
