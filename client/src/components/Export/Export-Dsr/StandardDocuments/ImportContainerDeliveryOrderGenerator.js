...
const ImportContainerDeliveryOrderGenerator = ({ jobNo, children, onTrackSuccess }) => {
  const generatePDF = async (e) => {
    ...
    try {
      ...
      const firstVehicle = data.operations?.[0]?.transporterDetails?.[0]?.vehicleNo || "";
      const firstTransporter = data.operations?.[0]?.transporterDetails?.[0]?.transporterName || "";
      ...
      containers.forEach((c, index) => {
        ...
        const vNo = c.weighmentVehicleNo || trans?.vehicleNo || firstVehicle || "";
        const tName = c.weighmentTransporterName || trans?.transporterName || firstTransporter || "";

        const containerNo = c.containerNo || c.container_number || "";
        const isoCode = c.isoCode || "";
        const grWt = c.grossWeight ? `${c.grossWeight} KGS` : `${data.gross_weight_kg || ""} KGS`;
        ...
        const tableBody = [
          [
            { content: `Vehicle\n\n${vNo}`, styles: { fontStyle: "bold" } },
            { content: `Driver/Transporter\n\n${tName}`, styles: { fontStyle: "bold" } },
        ...
        if (isGandhidham) {
          ...
          doc.setFont("helvetica", "bold");
          doc.setFontSize(8.5);
          doc.text("AUTHORIZED SIGNATURE", pageWidth - 85, tempY);
          doc.setTextColor(0, 0, 0); // Reset color
        } else {
