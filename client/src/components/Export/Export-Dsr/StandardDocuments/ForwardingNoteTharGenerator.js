...
const ForwardingNoteTharGenerator = ({ jobNo, children }) => {
  ...
  const getAggregatedContainers = (primaryJob, isClubActive, clubbedJobsList) => {
    ...
    const primaryContainers = primaryJob.containers?.length > 0
      ? primaryJob.containers
      : (primaryJob.operations?.[0]?.containerDetails || []);
    const invoice = primaryJob.invoices?.[0] || {};
    const products = invoice.products || [];
    const hsnListPrimary = [...new Set(products.map(p => {
    ...
    const descriptionOfGoodsPrimary = products[0]?.description || "";
    ...
    primaryContainers.forEach(c => {
      result.push({
        ...
        descriptionOfGoods: descriptionOfGoodsPrimary,
        hsnList: hsnListPrimary,
        shippingBillNo: primaryJob.shippingBillNo || primaryJob.sb_no || "",
        sb_date: primaryJob.sb_date || "",
        invoiceNumber: invoice.invoiceNumber || "",
        invoiceValue: invoice.invoiceValue || "",
        leoDate: primaryJob.operations?.[0]?.statusDetails?.[0]?.leoDate || "",
      });
    });

    if (isClubActive && Array.isArray(clubbedJobsList)) {
      clubbedJobsList.forEach(cJobData => {
        const cJobContainers = cJobData.containers?.length > 0
          ? cJobData.containers
          : (cJobData.operations?.[0]?.containerDetails || []);
        const cJobInvoice = cJobData.invoices?.[0] || {};
        const cJobProducts = cJobInvoice.products || [];
        const cJobHsnList = [...new Set(cJobProducts.map(p => {
          if (p.hsn_code || p.hsnCode || p.hsn) return p.hsn_code || p.hsnCode || p.hsn;
          if (p.ritc) {
            if (typeof p.ritc === 'object') return p.ritc.hsnCode || p.ritc.ritcCode;
            return p.ritc;
        ...
        const cJobDescription = cJobProducts[0]?.description || "";
        ...
        cJobContainers.forEach(c => {
          result.push({
            ...
            hsnList: cJobHsnList,
            shippingBillNo: cJobData.shippingBillNo || cJobData.sb_no || "",
            sb_date: cJobData.sb_date || "",
            invoiceNumber: cJobInvoice.invoiceNumber || "",
            invoiceValue: cJobInvoice.invoiceValue || "",
            leoDate: cJobData.operations?.[0]?.statusDetails?.[0]?.leoDate || "",
          });
        });
      });
    }
  ...
  const buildTemplate = (primaryData, isClubActive, clubbedJobsList, logo) => {
    ...
    aggregatedContainers.forEach((c, i) => {
      ...
      const pkgsDisplay = c.uniqueSBs && c.uniqueSBs.length > 0
        ? c.uniqueSBs.map(sb => `<div>${sb.pkgs || ""}<br/>&nbsp;</div>`).join("<div style='height: 8px;'></div>")
        : (pkgs ? `<div>${pkgs}</div>` : "");

      const weightDisplay = c.uniqueSBs && c.uniqueSBs.length > 0
        ? c.uniqueSBs.map(sb => `<div>${sb.weight || ""}<br/>&nbsp;</div>`).join("<div style='height: 8px;'></div>")
        : `<div>${weight}</div>`;

      const tareWeightDisplay = c.uniqueSBs && c.uniqueSBs.length > 0
        ? c.uniqueSBs.map(sb => `<div>${sb.tareWeight || ""}<br/>&nbsp;</div>`).join("<div style='height: 8px;'></div>")
        : `<div>${c.tareWeightKgs || ""}</div>`;

      const sbDisplay = c.uniqueSBs && c.uniqueSBs.length > 0
        ? c.uniqueSBs.map(sb => `<div>${sb.sbNo}<br/>dt ${formatDate(sb.sbDate)}</div>`).join("<div style='height: 8px;'></div>")
    ...
    const totalPkgs = aggregatedContainers.reduce((sum, c) => sum + (Number(c.pkgsStuffed) || 0), 0);
    const totalGrossWeight = aggregatedContainers.reduce((sum, c) => sum + (Number(c.grossWeight) || 0), 0);
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      aggregatedContainers.forEach((c, i) => {
        ...
        const pkgsText = c.uniqueSBs && c.uniqueSBs.length > 0
          ? c.uniqueSBs.map(sb => sb.pkgs || "").join("\n")
          : (pkgs || "");
        ...
        const weightText = c.uniqueSBs && c.uniqueSBs.length > 0
          ? c.uniqueSBs.map(sb => sb.weight || "").join("\n")
          : (weight || "");
        worksheet.getCell(currentRow, 6).value = weightText;

        const tareWeightText = c.uniqueSBs && c.uniqueSBs.length > 0
          ? c.uniqueSBs.map(sb => sb.tareWeight || "").join("\n")
          : (tareWeight || "");
        ...
        const sbText = c.uniqueSBs && c.uniqueSBs.length > 0
          ? c.uniqueSBs.map(sb => `${sb.sbNo}\ndt ${formatDate(sb.sbDate)}`).join("\n\n")
