...
const GeneralTab = ({ formik, directories }) => {
  ...
  useEffect(() => {
    ...
    if (exp) {
      ...
      const shouldUpdate =
        getVal("ie_code_no") !==
          toUpper(exp.registrationDetails?.ieCode || "") ||
        getVal("gstin") !==
          toUpper(exp.registrationDetails?.gstinMainBranch || "") ||
        getVal("exporter_address") !==
          toUpper(
            `${exp.branchInfo?.[0]?.address || ""}${
              exp.branchInfo?.[0]?.postalCode
                ? `, ${exp.branchInfo?.[0]?.postalCode}`
                : ""
            }`
          );
      ...
      if (shouldUpdate) {
        ...
        const updates = {
          ...
          gstinMainBranch: toUpper(
            exp.registrationDetails?.gstinMainBranch || ""
          ),
          regn_no: toUpper(exp.registrationDetails?.gstinMainBranch || ""),
          exporter_gstin: toUpper(
            exp.registrationDetails?.gstinMainBranch || ""
          ),
        };
